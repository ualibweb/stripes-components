...
import { asyncGenerate, syncGenerate, syncGenerateObject, asyncGenerateObject } from './service';
...
export default class OffsetPaging extends React.Component {
  constructor() {
    ...
    this.state = {
      data: {},
      queue: [{ start: 0, data: syncGenerateObject(4, 4) }], // eslint-disable-line
      widths: {
        index: '100px',
        title: '150px',
        date: '200px',
  ...
  requestMore = async (amount, index, first) => {
    ...
    const newData = await asyncGenerateObject(amount, index, 1000);
  ...
  addSome = () => {
    this.setState(curState => {
      const newQueue = syncGenerateObject(6, curState.data.length);
      return {
        data: { ...curState.data, length: curState.data.length + 6 },
  ...
  reset = () => {
    this.setState(curState => {
      return {
        ...
        queue: [{ start: 0, data: syncGenerateObject(4, 0) }], // eslint-disable-line
        mclKey: curState.mclKey + 1,
        first: 0
      };
    });
